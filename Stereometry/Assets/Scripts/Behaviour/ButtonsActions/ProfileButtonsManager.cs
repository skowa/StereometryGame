    ...
    public class ProfileButtonsManager : MonoBehaviour
    {
        ...
        private async Task OnAuthenticationFinished(Task<User> task)
        {
            if (task.IsCompleted)
            {
                ...
                if (PlayerPrefs.HasKey("level") && PlayerPrefs.GetInt("level") < user.LevelsPassed + 1)
                {
                    PlayerPrefs.SetInt("level", user.LevelsPassed + 1);
                }
        ...
        {
            _accountText.text =
                $"{PlayerPrefs.GetString("email")}{Environment.NewLine}{PlayerPrefs.GetString("name")}{Environment.NewLine}Levels: {PlayerPrefs.GetInt("level")}";
